...
export default function App() {
  ...
  useEffect(() => { if (lesson.data) { setVoltage(lesson.data.activity.voltage.value); setResistance(lesson.data.activity.resistance.value); } }, [lesson.data]);
  useEffect(() => { setAttemptId(undefined); setHint(undefined); setRevealedAnswer(undefined); setTransferPrompt(undefined); }, [mode]);
  ...
  const attempt = useMutation({
    ...
    onSuccess: (result) => { setAttemptId(result.attemptId); setNotice(undefined); void queryClient.invalidateQueries({ queryKey: ["home"] }); },
    onError: (error) => setNotice(error instanceof Error ? error.message : "Could not check the answer."),
  });
  const hintMutation = useMutation({
    mutationFn: async () => { if (!attemptId) throw new Error("Submit an attempt before asking for a hint."); return requestHint(attemptId); },
    onSuccess: (result) => setHint(result.hint),
    onError: (error) => setNotice(error instanceof Error ? error.message : "Could not load a hint."),
  });

  function speakLesson(): void {
    if (!lesson.data || !("speechSynthesis" in window)) { setNotice("Speech synthesis is unavailable in this browser."); return; }
